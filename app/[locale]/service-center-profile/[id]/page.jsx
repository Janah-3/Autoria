...
export default function CenterProfilePage() {
  ...
  const t = useTranslations("centerProfile");
  ...
  const handleReportSubmit = async (e) => {
    e.preventDefault();
    if (!user) { triggerToast(t("toastLoginRequired"), "error"); return; }
    if (!reason) { triggerToast(t("toastSelectReason"), "error"); return; }
    ...
    try {
      ...
      triggerToast(t("toastReportSuccess"), "success");
      setReportDropdownOpen(false);
      setReason("");
      setDetails("");
    } catch (err) {
      triggerToast(err.message || t("toastReportFail"), "error");
    } finally {
  ...
  useEffect(() => {
    getMe().then(res => { if (res?.data?.fullName) setUser({ name: res.data.fullName }); }).catch(() => {});

    serviceCentersService.getById(params.id)
      .then(res => {
        const d = res?.data ?? res;
        if (d && (d.name || d.Name)) {
          setCenter(mapServiceCenterListItem(d));
        } else {
          setCenter(MOCK_CENTERS[params.id] ?? null);
        }
      })
      .catch(() => setCenter(MOCK_CENTERS[params.id] ?? null))
      .finally(() => setLoading(false));

    reviewsService.getServiceCenterReviews(params.id)
  ...
  if (loading) return (
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f7f7f8" }}>
      <div style={{ color: R, fontSize: 18, fontWeight: 700 }}>{t("loading")}</div>
    </div>
  );

  if (!center) return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#f7f7f8" }}>
      <h2 style={{ fontSize: 24, fontWeight: 900, marginBottom: 12 }}>{t("notFound")}</h2>
      <a href="/service-centers" style={{ color: R, fontWeight: 700, textDecoration: "none" }}>{t("backToCenters")}</a>
    </div>
  );

  const avgRating = reviews.length > 0
    ? (reviews.reduce((acc, r) => acc + (r.rating ?? r.Rating ?? 0), 0) / reviews.length).toFixed(1)
    : "0.0";
  const reviewLabel = reviews.length === 1
  ...
  return (
    <div style={{ fontFamily: "'Inter',sans-serif", background: "#f7f7f8", minHeight: "100vh", color: "#111" }}>
      ...
      <div style={{
        height: 320,
        background: center.coverPhoto
          ? `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.6)), url(${center.coverPhoto}) center/cover no-repeat`
        ...
        <div>
          <Section title={t("sectionProfile")}>
            <p style={{ color: "#6b7280", lineHeight: 1.8, fontSize: 15 }}>
              {t("profileDescTemplate", {
                name: center.name,
                type: center.type || t("typeDefault"),
                district: center.district,
                brands: center.carBrands?.join(", "),
              })}
            </p>
          </Section>
          ...
          <Section title={t("sectionServices")}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {center.serviceTypes?.map(service => (
                <div key={service} style={{ background: "#fff", padding: "16px", borderRadius: 12, border: "1px solid #e5e7eb", display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#fff0f0", color: R, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>✓</div>
          ...
          <Section title={t("sectionReviews")}>
            {reviewsLoading ? (
              <div style={{ color: "#6b7280", fontSize: 14 }}>{t("reviewsLoading")}</div>
            ) : reviews.length === 0 ? (
              <div style={{ color: "#6b7280", fontSize: 14, fontStyle: "italic" }}>{t("noReviews")}</div>
            ) : (
                  ...
                  <div key={r.id || r.Id} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 20 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      <div>
                        <span style={{ color: "#f59e0b", fontSize: 13, letterSpacing: 1 }}>
                          {"★".repeat(Number(r.rating) || Number(r.Rating) || 5)}
                          {"☆".repeat(5 - (Number(r.rating) || Number(r.Rating) || 5))}
                        </span>
                        <span style={{ background: "#fff0f0", color: R, fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20, marginLeft: 6 }}>
                          <i className="fa-solid fa-check" style={{ marginRight: 3 }}></i>{t("verified")}
                        </span>
                      </div>
                    ...
                    {(r.replyComment || r.ReplyComment) && (
                      <div style={{ marginTop: 14, background: "#f9fafb", borderRadius: 8, padding: 12, borderLeft: "3px solid #6b7280" }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", marginBottom: 4 }}>{t("replyFromOwner")}</div>
          ...
          <div style={{ marginTop: 20 }}>
            <a href="/gallery" style={{ textDecoration: "none" }}>
              <button className="btn-hover" style={{ background: "#fff", color: R, border: `2px solid ${R}`, padding: "12px 24px", borderRadius: "12px", fontSize: "14px", fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", gap: "10px" }}>
                <span>📷</span> {t("viewGallery")}
              </button>
            </a>
          </div>
        </div>
          ...
          <div style={{ background: "#fff", borderRadius: 20, border: "1.5px solid #e5e7eb", padding: 24, boxShadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
            ...
            {!isServiceCenterOrAdmin && (
              <Link href={`/book-service?serviceCenterId=${center.id}`} style={{ textDecoration: "none" }}>
                <button className="btn-hover" style={{ width: "100%", background: R, color: "#fff", border: "none", padding: "14px 20px", borderRadius: "12px", fontWeight: 800, fontSize: "15px", cursor: "pointer", marginBottom: "24px", boxShadow: "0 4px 14px rgba(232,39,42,0.25)", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
                  <i className="fa-solid fa-calendar-check"></i> {t("bookNow")}
                </button>
              </Link>
            )}

            <h3 style={{ fontSize: 18, fontWeight: 800, marginBottom: 4 }}>{t("contactInfo")}</h3>
            <p style={{ fontSize: 12, color: "#9ca3af", marginBottom: 20 }}>{t("contactSubtitle")}</p>

            {/* Phone */}
            <div style={{ marginBottom: 16, padding: "14px 16px", background: "#f9fafb", borderRadius: 12, border: "1px solid #f3f4f6" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>{t("phoneLabel")}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ color: R, fontSize: 16 }}>📞</span>
                <span style={{ fontSize: 16, fontWeight: 700, color: R }}>{center.phone || t("phoneNotProvided")}</span>
              </div>
            </div>

            {/* Location */}
            <div style={{ marginBottom: 16, padding: "14px 16px", background: "#f9fafb", borderRadius: 12, border: "1px solid #f3f4f6" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>{t("locationLabel")}</div>
...
>
  <span>🗺️</span> {t("viewOnMaps")}
            ...
            <div style={{ marginBottom: 20, padding: "14px 16px", background: "#f9fafb", borderRadius: 12, border: "1px solid #f3f4f6" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>{t("ratingLabel")}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ color: "#f59e0b", fontSize: 16 }}>★</span>
                <span style={{ fontSize: 15, fontWeight: 700, color: "#374151" }}>{avgRating}</span>
                <span style={{ fontSize: 13, color: "#9ca3af" }}>({reviewLabel})</span>
              </div>
            </div>
            ...
            {!isServiceCenterOrAdmin && (
              <button
                ...
                className="btn-hover"
                style={{ width: "100%", background: "#0d0202", color: "#fff", border: "none", padding: "12px", borderRadius: "12px", fontWeight: 800, fontSize: "14px", cursor: "pointer", transition: "all 0.2s" }}
              >
                {t("reportCenter")}
              </button>
            )}
      ...
      <div style={{
        ...
        border: `1.5px solid ${toast.type === "success" ? "#C3E6CB" : "#F5C6CB"}`,
      ...
      {reportDropdownOpen && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, backdropFilter: "blur(4px)" }}>
          <div style={{ background: "#fff", borderRadius: "20px", padding: "30px", width: "420px", boxShadow: "0 10px 35px rgba(0,0,0,0.15)", border: "1px solid #e5e7eb", color: "#111" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              <h3 style={{ fontSize: "18px", fontWeight: 800, margin: 0, display: "flex", alignItems: "center", gap: 8 }}>
                <span>⚠️</span> {t("reportTitle")}
              </h3>
              <button onClick={() => setReportDropdownOpen(false)} style={{ background: "none", border: "none", fontSize: "20px", cursor: "pointer", color: "#9ca3af" }}>✕</button>
            </div>

            {!user ? (
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                <span style={{ fontSize: 32 }}>🔒</span>
                <h4 style={{ fontSize: 16, fontWeight: 700, marginTop: 12, marginBottom: 8 }}>{t("loginRequired")}</h4>
                <p style={{ fontSize: 13, color: "#6b7280", lineHeight: 1.5, marginBottom: 20 }}>{t("loginRequiredDesc")}</p>
                <div style={{ display: "flex", gap: 10 }}>
                  <Link href="/login" style={{ flex: 1, textDecoration: "none", background: R, color: "#fff", padding: "10px", borderRadius: 8, fontWeight: 700, fontSize: 13, display: "inline-block", textAlign: "center" }}>
                    {t("logIn")}
                  </Link>
                  <button onClick={() => setReportDropdownOpen(false)} style={{ flex: 1, background: "#fff", color: "#111", border: "1px solid #e5e7eb", padding: "10px", borderRadius: 8, fontWeight: 600, fontSize: 13, cursor: "pointer" }}>
                    {t("reportCancel")}
                  </button>
                </div>
              </div>
              ...
              <form onSubmit={handleReportSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <p style={{ fontSize: 13, color: "#6b7280", margin: 0, lineHeight: 1.5 }}>
                  {t("reportingCenter")} <strong>{center.name}</strong>.
                </p>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <label style={{ fontSize: 11, fontWeight: 800, color: "#374151", textTransform: "uppercase", letterSpacing: 0.5 }}>
                    {t("reportSelectReason")}
                  </label>
                  <select
                    value={reason}
                  ...
                  >
                    <option value="">{t("reportChooseReason")}</option>
                    <option value="Spam">{t("reportReasons.spam")}</option>
                    <option value="Inappropriate">{t("reportReasons.inappropriate")}</option>
                    <option value="Fake">{t("reportReasons.fake")}</option>
                    <option value="Offensive">{t("reportReasons.offensive")}</option>
                ...
                <div style={{ display: "flex", gap: 12, marginTop: 8 }}>
                  ...
                  <button
                    ...
                    style={{ flex: 1, background: submittingReport ? RD : R, color: "#fff", border: "none", padding: "11px", borderRadius: 8, fontWeight: 800, fontSize: 13, cursor: submittingReport ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
                  >
                    {submittingReport ? t("reportSubmitting") : t("reportSubmit")}
                  </button>
                </div>
              </form>
            )}
