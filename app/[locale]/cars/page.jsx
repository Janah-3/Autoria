...
const CarCard = ({ car, handleSetDefault }) => {
  const tCars   = useTranslations("cars");
  const tCommon = useTranslations("common");

  const fuelKey = car.fuelType !== undefined ? String(car.fuelType) : "Petrol";
  const transKey = car.transmission !== undefined ? String(car.transmission) : "Manual";
  const fuelName = tCars(`fuels.${fuelKey}`);
  const transmissionName = tCars(`transmissions.${transKey}`);
    ...
    <div className={`car-card ${car.isPrimary ? "default" : ""}`}>
      ...
      <div className="car-body">
        <div className="car-top">
          ...
          {car.isPrimary && (
            <div className="default-badge">{tCars("defaultBadge")}</div>
          )}
        </div>
      ...
      <div className="car-actions">
        ...
        <Link
          href={`/cars/deleteCar?id=${getCarId(car)}`}
...
export default function CarsPage() {
  ...
  const handleSetDefault = async (car) => {
    try {
      ...
      displayToast(tCars("defaultBadge"));
      loadCars();
    } catch (error) {
      alert(tCars("failedToUpdate"));
    }
  };
    ...
    <div className="cars-page-container">
      <style>{`
        :root {
          ...
          --shadow-red: 0 8px 28px rgba(232,25,44,.25);
          --r: 16px; 
          --r-sm: 10px; 
        ...
        * { box-sizing: border-box; }
        ...
        .btn-edit, .btn-set-default { flex: 1; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; cursor: pointer; }
        .btn-edit { border: 1.5px solid var(--g200); color: var(--g700); }
      ...
      <div className="page">
        ...
        <div className="page-header">
          <div>
            <div className="page-title">{tCars("title")}</div>
            <div style={{ fontSize: "14px", color: "#999" }}>
              {carsList.length} {tCars("vehicles")}
            </div>
          </div>
        ...
        {/* CARS GRID */}
        {isLoading ? (
          <div className="loading">{tCars("loading")}</div>
        ) : (
          <div className="cars-grid">
            {carsList.map(car => (
              <CarCard
                key={car.carId || car.id || car.vin}
                car={car}
            ...
            <Link href="/cars/add-car" className="add-card" style={{ textDecoration: "none" }}>
              ...
              <div style={{ fontWeight: "700", color: "#666" }}>{tCars("addNewCard")}</div>
            </Link>
          </div>
        )}
