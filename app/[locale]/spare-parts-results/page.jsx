      ...
      <div style={{ padding: "18px" }}>
        ...
        {part.lowestPrice ? (
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "20px", fontWeight: 800, color: COLORS.accent, marginBottom: "4px" }}>
             {part.lowestPrice} <span style={{ fontSize: "13px", marginLeft: "4px" }}>EGP</span>
          </div>
        ) : (
...
function ResultsContent() {
  ...
  const categories = [
    { value: "All",               label: tTypes("All") },
    { value: "Brakes",            label: tTypes("Brakes") },
    { value: "Engine",            label: tTypes("Engine") },
    { value: "Filters",           label: tTypes("Filters") },
    { value: "Electrical",        label: tTypes("Electrical") },
    { value: "Suspension",        label: tTypes("Suspension") },
    { value: "Air Conditioning",  label: tTypes("Air Conditioning") },
  ];
    ...
    <div style={{ background: COLORS.bg, minHeight: "100vh", color: COLORS.text, fontFamily: "'Inter', sans-serif" }}>
      ...
      <section style={{ background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, padding: "40px" }}>
        <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "32px", fontWeight: 800, marginBottom: "6px", color: COLORS.text }}>
            {tNav("spareParts")}
          </h1>
          <p style={{ color: COLORS.muted2, fontSize: "14px", marginBottom: "28px" }}>
            {tHome("spareParts.subtitle")}
          </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 180px 180px 180px auto", gap: "12px", alignItems: "end", background: "#F8F9FA", padding: "20px", borderRadius: "16px", border: `1px solid ${COLORS.border}` }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
               <label style={{ fontSize: "10px", fontWeight: 700, color: COLORS.muted2, textTransform: "uppercase" }}>
                 {tNav("spareParts")}
               </label>
               <input 
                 value={q} 
      ...
      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", maxWidth: "1400px", margin: "0 auto" }}>
        <aside style={{ padding: "30px", borderRight: `1px solid ${COLORS.border}`, background: "#FFF" }}>
          <div style={{ fontSize: "11px", fontWeight: 800, color: COLORS.muted2, letterSpacing: "1px", marginBottom: "30px", textTransform: "uppercase" }}>
            {tCommon("Filters")}
          </div>
          
          <div style={{ marginBottom: "35px" }}>
            <div style={{ fontSize: "11px", fontWeight: 900, marginBottom: "15px", color: COLORS.text }}>
              {tCommon("Filters")}
            </div>
            {categories.map(cat => (
          ...
          <div style={{ marginBottom: "35px" }}>
            ...
            >
  <option value="Cairo">{tLoc("Cairo")}</option>
  <option value="Giza">{tLoc("Giza")}</option>
  <option value="Alexandria">{tLoc("Alexandria")}</option>
</select>
          </div>
        </aside>
        ...
        <div style={{ padding: "30px" }}>
          <div style={{ display: "flex", gap: "8px", alignItems: "center", fontSize: "12px", color: COLORS.muted2, marginBottom: "25px" }}>
            ...
            {selectedCategories.length > 0 ? (
              ...
              (q && q.toLowerCase().trim() !== "all") && <> <span style={{ color: COLORS.muted }}>/</span> <span style={{ color: COLORS.text, fontWeight: 700 }}>{q}</span> </>
            )}
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "25px" }}>
             {loading ? (
               <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "50px", color: COLORS.muted2 }}>
                 {tCommon("loading")}
               </div>
             ) : parts.length > 0 ? (
