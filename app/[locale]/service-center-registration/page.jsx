...
export default function ServiceCenterRegistration() {
  ...
  const tServiceCenters = useTranslations("serviceCenters");
          ...
          {currentStep === 1 && (
            <div>
              ...
              <div className="form-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                ...
                <div style={{ gridColumn: "span 2" }}>
                  <label className="form-label">{tServiceCenters("centerType")}</label>
                  <select className="form-input" value={basicInfo.type} onChange={e => setBasicInfo(p => ({ ...p, type: parseInt(e.target.value) }))}>
                    <option value={0}>{tAdmin("maintenanceCenter")}</option>
                    <option value={1}>{tAdmin("partsStore")}</option>
                    <option value={2}>{tAdmin("hybridCenter")}</option>
                  </select>
                </div>
                      ...
                      {availableServices.map(s => {
                        ...
                        return (
                          <div key={s.id} className={`tag-chip${on ? " selected" : ""}`}
                            ...
                            {on && <span style={{ fontSize: 10 }}>✓</span>}{tTypes.has(s.name) ? tTypes(s.name) : s.name}
                          </div>
                        );
                      })}
                    </div>
