  ...
  return (
    <section style={{ background: `linear-gradient(135deg,#111 0%,#2d1010 52%,${RD} 100%)`, padding: "72px 5% 64px", textAlign: "center" }}>
      <h1 style={{ color: "#fff", fontSize: 44, fontWeight: 900, lineHeight: 1.12, letterSpacing: -1.5, marginBottom: 14 }}>
        {t("title").replace(t("titleEm"), "")}
        <em style={{ color: "#ff6b6b", fontStyle: "normal" }}>{t("titleEm")}</em>
      </h1>
        ...
        <div style={{ background: "#fff", borderRadius: 12, padding: "6px 6px 6px 0", display: "flex", alignItems: "center", flex: 1, minWidth: "300px", boxShadow: "0 4px 24px rgba(0,0,0,.2)" }}>
          <div style={{ flex: 1, ...row(8), padding: "0 16px", borderRight: "1px solid #e5e7eb" }}>
            ...
            <input
              ...
              placeholder={t("searchPlaceholderService")}
          ...
          <div style={{ flex: 1, ...row(8), padding: "0 16px" }}>
            ...
            <input
              ...
              placeholder={t("searchPlaceholderLoc")}
              style={{ border: "none", outline: "none", width: "100%", fontSize: 14, color: "#374151", background: "transparent" }}
            />
          </div>
          <button
...
function ServiceCenters({ centers }) {
  const t = useTranslations("home.serviceCenters");
  const tDynamic = useTranslations("home");

  return (
    <section style={{ padding: "72px 5%", background: "#f7f7f8" }}>
      <SH tag={t("tag")} h2={t("title")} em={t("titleEm")} sub={t("subtitle")} />
      <div style={grid(3, 18)}>
        {centers.map(c => (
            ...
            <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, overflow: "hidden", cursor: "pointer" }}>
              ...
              <div style={{ padding: 15 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 11 }}>
                  {c.tags.map(tag => <span key={tag} style={{ background: "#f3f4f6", color: "#374151", fontSize: 10, padding: "3px 8px", borderRadius: 6, fontWeight: 600 }}>{tag}</span>)}
                </div>
      ...
      <div style={{ textAlign: "center", marginTop: 28 }}>
        <a href="/service-centers" style={{ textDecoration: "none" }}>
          <button className="btn-hover" style={{ background: "transparent", border: `1.5px solid ${R}`, color: R, padding: "10px 26px", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{t("viewAllBtn")}</button>
        </a>
      </div>
    </section>
  );
...
const SPARE_PART_CATEGORIES = [
  { iconClass: "fa-solid fa-circle-notch", titleKey: "Brakes",            searchKey: "Brakes" },
  { iconClass: "fa-solid fa-gears",        titleKey: "Engine Repair",     searchKey: "Engine Repair" },
  { iconClass: "fa-solid fa-filter",       titleKey: "Diagnostics",       searchKey: "Diagnostics" },
  { iconClass: "fa-solid fa-bolt",         titleKey: "Electrical",        searchKey: "Electrical" },
  { iconClass: "fa-solid fa-compress",     titleKey: "Suspension",        searchKey: "Suspension" },
  { iconClass: "fa-solid fa-snowflake",    titleKey: "AC Repair",         searchKey: "AC Repair" },
];
function SpareParts() {
  const t = useTranslations("home.spareParts");
    ...
    <section style={{ padding: "72px 5%" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: "16px" }}>
        {SPARE_PART_CATEGORIES.map((cat, idx) => {
          ...
          const label = tTypes.has(cat.titleKey) ? tTypes(cat.titleKey) : cat.titleKey;
          return (
            <div
              key={cat.titleKey}
              onClick={() => router.push(`/spare-parts-results?q=${encodeURIComponent(cat.titleKey)}`)}
            ...
            >
              ...
              <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "14px", fontWeight: 700, marginBottom: "4px", color: "#111111" }}>{label}</div>
            </div>
          );
        })}
      </div>
      <div style={{ textAlign: "center", marginTop: 28 }}>
        <a href="/spare-parts-search" style={{ textDecoration: "none" }}>
          <button className="btn-hover" style={{ background: "transparent", border: `1.5px solid ${R}`, color: R, padding: "10px 26px", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{t("shopAllBtn")}</button>
        </a>
      </div>
    </section>
  );
      ...
      <div style={grid(3, 16)}>
        {steps.map((st, i) => (
          <div key={i} style={{ textAlign: "center", padding: "0 16px" }}>
            <div style={{ width: 72, height: 72, borderRadius: "50%", background: R, color: "#fff", fontSize: 26, fontWeight: 900, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 18px", border: "4px solid #fff0f0", boxShadow: "0 4px 16px rgba(232,39,42,.3)" }}>{i + 1}</div>
...
function WhyAutoria() {
  const t = useTranslations("home.whyAutoria");
        ...
        {items.map((w, i) => (
          <div key={i} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 22 }}>
            <div style={{ width: 42, height: 42, background: "#fff0f0", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 13, color: R }}>
              <i className={WHY_ICONS[i]} style={{ fontSize: "18px" }}></i>
            </div>
          ...
          <div key={r.name} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 20 }}>
            <div style={{ marginBottom: 10 }}>
              ...
              <span style={{ background: "#fff0f0", color: R, fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20, marginLeft: 6 }}>
                <i className="fa-solid fa-check" style={{ marginRight: 3 }}></i>{tCommon("verified")}
              </span>
            </div>
...
function CTA() {
  ...
  return (
    <section style={{ background: R, padding: "72px 5%", textAlign: "center" }}>
      <SH tag={t("tag")} h2={t("title")} em={t("titleEm")} sub={t("subtitle")} dark />
      <div style={{ ...row(12), justifyContent: "center", marginTop: 28 }}>
        <a href="/book-service" style={{ textDecoration: "none" }}>
          <button className="btn-hover" style={{ background: "#fff", color: R, border: "none", padding: "13px 28px", borderRadius: 9, fontSize: 13, fontWeight: 800, cursor: "pointer" }}>{t("bookBtn")}</button>
        </a>
      </div>
    </section>
  );
...
// ── Main export ─────────────────────────────────────────────────────────────
export default function AutoriaHomePage({ navTranslations }) {


  const t = useTranslations("home.aiModal");
  ...
  const handleAiMatchSubmit = async (e) => {
    e.preventDefault();
    if (!aiIssue) { alert(t("errorAlert")); return; }
  ...
  useEffect(() => {
    ...
    serviceCentersService.getAll()
      .then(res => { const items = getServiceCenterItems(res); if (items.length) setCenters(items); })
      .catch(() => {});

    sparePartsService.getSpareParts()
      .then(data => { if (data.length) setParts(data); })
      .catch(() => {});
  }, []);
  ...
  return (
    <div style={{ fontFamily: "'Inter',sans-serif", color: "#111", background: "#fff", lineHeight: 1.6 }}>
      ...
      `}</style>
      <Navbar user={user} translations={navTranslations} />
          ...
          <div style={{ background: "rgba(255, 255, 255, 0.95)", borderRadius: "20px", width: "100%", maxWidth: "650px", padding: "32px", border: "1.5px solid rgba(255, 255, 255, 0.3)", boxShadow: "0 20px 50px rgba(0, 0, 0, 0.15)", maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", borderBottom: "1.5px solid #F1F5F9", paddingBottom: "12px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                ...
                <h3 style={{ fontSize: "20px", fontWeight: 800, margin: 0, color: "#111111" }}>{t("title")}</h3>
              </div>
              <button
                onClick={() => { setShowAiModal(false); setAiMatches([]); setAiInterpreted(null); setAiIssue(""); }}
            ...
            <form onSubmit={handleAiMatchSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                <label style={{ fontSize: "13px", fontWeight: 700, color: "#475569" }}>{t("label")}</label>
                <textarea
                  value={aiIssue}
              ...
              <div style={{ display: "flex", gap: "12px", alignItems: "flex-end" }}>
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "6px" }}>
                  <label style={{ fontSize: "12px", fontWeight: 700, color: "#475569" }}>{t("lat")}</label>
                  <input type="text" value={aiLat} onChange={(e) => setAiLat(e.target.value)} placeholder="30.0444" style={{ padding: "10px 14px", border: "1.5px solid #E2E8F0", borderRadius: "8px", fontSize: "13px", background: "#F8FAFC" }} />
                </div>
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "6px" }}>
                  <label style={{ fontSize: "12px", fontWeight: 700, color: "#475569" }}>{t("lng")}</label>
                  <input type="text" value={aiLng} onChange={(e) => setAiLng(e.target.value)} placeholder="31.2357" style={{ padding: "10px 14px", border: "1.5px solid #E2E8F0", borderRadius: "8px", fontSize: "13px", background: "#F8FAFC" }} />
                </div>
                <button
                  type="button"
                  onClick={handleGetLocation}
                  disabled={gettingLocation}
                  style={{ background: "#F1F5F9", color: "#475569", border: "1.5px solid #CBD5E1", height: "39px", padding: "0 16px", borderRadius: "8px", fontSize: "12.5px", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" }}
                >
                  <i className="fa-solid fa-location-crosshairs" style={{ color: "#E8272A" }}></i>
                  {gettingLocation ? "..." : t("getLocationBtn")}
                </button>
              </div>

              {saveStatus && (
                <div style={{ fontSize: "12px", fontWeight: 700, color: saveStatus === "saved" ? "#10B981" : saveStatus === "saving" ? "#3B82F6" : "#EF4444", display: "flex", alignItems: "center", gap: "6px", marginTop: "-8px" }}>
                  {saveStatus === "saving" && <><i className="fa-solid fa-spinner fa-spin"></i> {t("savingLoc")}</>}
                  {saveStatus === "saved" && <><i className="fa-solid fa-circle-check"></i> {t("savedLoc")}</>}
                  {saveStatus === "error" && <><i className="fa-solid fa-circle-exclamation"></i> {t("saveFailed")}</>}
                </div>
              )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                ...
                <input type="range" min="2" max="50" value={aiRadius} onChange={(e) => setAiRadius(parseInt(e.target.value))} style={{ width: "100%", accentColor: "#E8272A", cursor: "pointer" }} />
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "8px" }}>
                <button
                  ...
                  style={{ background: "#ffffff", color: "#111111", border: "1.5px solid #E6C687", padding: "12px 32px", borderRadius: "10px", fontSize: "14px", fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", gap: "8px", boxShadow: "0 4px 15px rgba(230, 198, 135, 0.25)" }}
                >
                  {matchingLoader ? (
                    <><div style={{ width: "16px", height: "16px", border: "2px solid #111111", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.6s linear infinite" }} />{t("analyzing")}</>
                  ) : (
                    <><i className="fa-solid fa-wand-magic-sparkles" style={{ color: "#E6C687" }}></i> {t("submitBtn")}</>
                  )}
                </button>
              </div>
            </form>
              ...
              <div style={{ marginTop: "24px" }}>
                {aiInterpreted && (
                  <div style={{ background: "linear-gradient(135deg, #FFF5F5 0%, #FFF0F0 100%)", border: "1px solid #FCA5A5", padding: "16px", borderRadius: "12px", marginBottom: "20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontSize: "11px", fontWeight: 800, color: "#B81C1F", textTransform: "uppercase", letterSpacing: "0.5px" }}>{t("analysisTitle")}</div>
                      <div style={{ fontSize: "14px", fontWeight: 800, color: "#1E1B4B", marginTop: "4px" }}>
                        {t("category")}: <span style={{ color: "#E8272A" }}>{aiInterpreted.serviceType || "General Maintenance"}</span>
                      </div>
                    </div>
                    <div>
                      <span style={{
                        background: aiInterpreted.urgency === "high" ? "#FEE2E2" : aiInterpreted.urgency === "medium" ? "#FEF3C7" : "#ECFDF5",
                        color: aiInterpreted.urgency === "high" ? "#991B1B" : aiInterpreted.urgency === "medium" ? "#92400E" : "#065F46",
                        fontSize: "11px", fontWeight: 800, padding: "5px 12px", borderRadius: "20px", textTransform: "uppercase"
                      }}>
                        🚨 {t("urgency")}: {aiInterpreted.urgency || "low"}
                      </span>
                    </div>
                  </div>
                )}

                <h4 style={{ fontSize: "15px", fontWeight: 800, color: "#1E293B", marginBottom: "12px" }}>{t("topMatches")}</h4>

                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                  {aiMatches.map((match, idx) => (
                    <div key={match.serviceCenterId} style={{ background: "#FFFFFF", border: "1.5px solid #E2E8F0", borderRadius: "14px", padding: "16px", display: "flex", flexDirection: "column", gap: "10px" }}>
                      ...
                      {match.explanation && (
                        <div style={{ marginLeft: "28px", background: "#F8FAFC", borderLeft: "3.5px solid #E8272A", padding: "8px 12px", borderRadius: "0 8px 8px 0" }}>
                          <p style={{ fontSize: "12.5px", color: "#475569", lineHeight: 1.5, margin: 0, fontStyle: "italic" }}>"{match.explanation}"</p>
                        </div>
                      )}
                    </div>
                  ))}
