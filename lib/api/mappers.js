...
export function mapBooking(item) {
  ...
  return {
    ...
    userId: item.userId ?? item.UserId ?? item.customerId ?? item.CustomerId ?? null,
    userName: item.userName ?? item.UserName ?? item.customerName ?? item.CustomerName ?? null,
...
export function mapServiceCenterListItem(item) {
  ...
  return {
    ...
    bg: "#f3f4f6",
    cover: item.coverPhoto ?? item.CoverPhoto ?? ((item.photos ?? item.Photos)?.[0] ?? null),
    open: true,
    ...
    governorate: gov,
    district: dist,
    coverPhoto: item.coverPhoto ?? item.CoverPhoto ?? ((item.photos ?? item.Photos)?.[0] ?? null),
  };
}
