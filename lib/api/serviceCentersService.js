...
export const serviceCentersService = {
  ...
  async updateCarBrands(carBrandIds) {
    const cleanIds = Array.isArray(carBrandIds)
      ? carBrandIds
    ...
    return apiAuthFetch(SERVICE_CENTER_ENDPOINTS.myCarBrands, {
      method: "PUT",
      body: JSON.stringify({ CarBrandIds: cleanIds }),
    });
  },
  ...
  async setMyLocation(payload) {
    const id = payload?.id || "00000000-0000-0000-0000-000000000000";
    const { id: _, ...body } = payload;
    return apiAuthFetch(`${SERVICE_CENTER_ENDPOINTS.myLocation}?id=${id}`, {
      method: "PUT",
